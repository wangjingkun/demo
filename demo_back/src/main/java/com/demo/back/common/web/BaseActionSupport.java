package com.demo.back.common.web;
...
public class BaseActionSupport {
	...
	/*
	 * public void sendAjaxMsg(String msg) { this.sendAjaxMsg(msg, request,
	 * response); }
	 */
	 ...
	 * HttpServletResponse response) { FileInputStream fin = null; OutputStream
	 * os = null; try { File f = new File(destFileName); if ((attachmentName ==
	 * null) || "".equals(attachmentName)) { attachmentName = f.getName(); }
	 * response.setContentType("application/vnd.ms-excel");
	 * response.setHeader("Content-Disposition", "attachment; filename=" +
	 * attachmentName + ".xls"); os = response.getOutputStream(); fin = new
	 * FileInputStream(f); IOUtils.copy(fin, os); os.flush(); os.close(); }
	 ...
	 */
	/*
	 * public String getLoginUserId(){ PermUser pu = getLoginUser(); if(null !=
	 * pu){ return pu.getUserName(); }else{ return ""; } }
	 * 
	 * protected boolean isBackUserLogin(){ return getLoginUser()!=null; }
	 * 
	 * protected PermUser getLoginUser(){ // PermUser pu =new PermUser(); //
	 * pu.setUserName("4028807b40e851100140e85110990000"); //
	 * pu.setRealName("aaaaaa1"); PermUser pu =
	 * (PermUser)getSession(SESSION_BACK_USER); return pu; }
	 */
